...
<template>
  <header class="fixed inset-x-0 top-0 z-50 border-b border-gray-200 bg-white/90 backdrop-blur-md dark:border-gray-800 dark:bg-gray-950/90">
    <div class="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 sm:px-6 lg:px-8">
      <router-link to="/" class="flex items-center gap-2 text-xl font-bold" aria-label="Fluxo home">
      ...
      <nav class="hidden items-center gap-8 text-sm font-medium text-gray-600 dark:text-gray-400 md:flex" aria-label="Main navigation">
        <router-link to="/#features" class="transition-colors hover:text-gray-900 dark:hover:text-gray-100">Features</router-link>
        ...
        <router-link to="/blog" :aria-current="active === 'blog' ? 'page' : undefined" :class="active === 'blog' ? 'text-blue-600 dark:text-blue-400' : 'hover:text-gray-900 dark:hover:text-gray-100'" class="transition-colors">Blog</router-link>
    ...
    <nav v-if="mobileMenuOpen" id="public-mobile-nav" class="space-y-2 border-t border-gray-200 bg-white px-4 py-4 text-sm dark:border-gray-800 dark:bg-gray-950 md:hidden" aria-label="Mobile navigation">
      <router-link to="/#features" class="block rounded-lg px-3 py-2.5 text-gray-600 hover:bg-gray-50 dark:text-gray-400 dark:hover:bg-gray-900" @click="mobileMenuOpen = false">Features</router-link>
      ...
      <router-link to="/blog" aria-current="page" class="block rounded-lg bg-blue-50 px-3 py-2.5 font-semibold text-blue-700 dark:bg-blue-950/50 dark:text-blue-300" @click="mobileMenuOpen = false">Blog</router-link>
