...
- Vue: use the existing components (`BaseModal`, `Card`, `AppButton`, `DataTable`, `TableActionMenu`, `ToggleSwitch`, etc.) before writing raw HTML
- Table row dropdowns must use `TableActionMenu` so scroll containers cannot clip them
