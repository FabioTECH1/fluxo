  ...
  <div v-if="site" class="flex gap-6">
    ...
    <div class="flex-1 space-y-6">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-100">
        ...
        <div v-if="deployments.length === 0" class="px-6 py-8 text-center text-gray-400 text-sm">
          No deployments yet.
        </div>
            ...
            <div class="flex items-start justify-between">
              <div class="flex-1">
                <div class="flex items-center gap-2">
                  ...
                  <span v-else class="font-mono text-sm font-medium text-gray-400">—</span>
                  <span :class="statusBadge(dep.status)">{{ dep.status }}</span>
                </div>
                <p class="text-xs text-gray-500 mt-1">Deployed {{ timeAgo(dep.created_at) }}</p>
              </div>
            </div>
          </li>
        </ul>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-100">
        ...
        <div v-if="daemons.length === 0" class="px-6 py-8 text-center text-gray-400 text-sm">
        ...
        <ul v-else class="divide-y divide-gray-100">
          <li v-for="d in daemons" :key="d.id" class="px-6 py-4">
            <div class="flex items-start justify-between">
              <div>
                <p class="text-sm font-medium text-gray-900">{{ d.command }}</p>
                <p class="text-xs text-gray-400 mt-0.5 font-mono">{{ d.directory || site.path }} &middot; {{ d.instances || 1 }} process</p>
              </div>
              <span :class="d.status === 'active' || d.status === 'running' ? 'text-green-600 bg-green-50 border border-green-200' : 'text-gray-500 bg-gray-50 border border-gray-200'" class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium">
                {{ d.status || 'unknown' }}
              </span>
            </div>
          </li>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-100">
        ...
        <div v-if="crons.length === 0" class="px-6 py-8 text-center text-gray-400 text-sm">
          No scheduled jobs.
        </div>
        <ul v-else class="divide-y divide-gray-100">
          <li v-for="c in crons" :key="c.id" class="px-6 py-4">
            <div>
              <p class="text-sm font-medium text-gray-900">{{ c.command }}</p>
              <p class="text-xs text-gray-400 mt-0.5">fluxo &middot; {{ c.expression }} &middot; {{ c.user || 'fluxo' }}</p>
            </div>
          </li>
        </ul>
      </div>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-100">
        ...
        <div v-if="activity.length === 0" class="px-6 py-8 text-center text-gray-400 text-sm">
          No recent activity.
        </div>
        <ul v-else class="divide-y divide-gray-100">
          <li v-for="(a, i) in activity.slice(0, 10)" :key="i" class="px-6 py-3">
            <div class="flex items-center gap-3">
              <span class="h-7 w-7 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-xs font-bold flex-shrink-0">
                {{ a.type === 'deployment' ? 'D' : 'S' }}
              </span>
              <div>
                <p class="text-sm text-gray-700">{{ a.summary }}</p>
    ...
    <div class="w-72 flex-shrink-0 space-y-4">
      <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-5">
        ...
        <div class="space-y-2.5">
          <div>
            <p class="text-xs text-gray-400">Server ID</p>
            <p class="text-sm font-mono text-gray-700">1000001</p>
          </div>
          <div>
            <p class="text-xs text-gray-400">Site ID</p>
            <p class="text-sm font-mono text-gray-700">{{ site.id }}</p>
          </div>
          <div>
            <p class="text-xs text-gray-400">Site User</p>
            <p class="text-sm font-mono text-gray-700">fluxo</p>
          </div>
          <div>
            <p class="text-xs text-gray-400">Framework</p>
            <p class="text-sm text-gray-700 capitalize">{{ site.app_type || 'php' }}</p>
          </div>
          <div>
            <p class="text-xs text-gray-400">PHP</p>
            <p class="text-sm font-mono text-gray-700">{{ site.php_version || '8.4' }}</p>
          </div>
          <div>
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-5 space-y-3">
        <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider">Features</h3>
        <div class="space-y-2">
          <div class="flex items-center justify-between text-sm">
            <span class="text-gray-600">Laravel Scheduler</span>
          ...
          <div class="flex items-center justify-between text-sm">
            <span class="text-gray-600">Octane</span>
            <span class="h-2 w-2 rounded-full bg-gray-300"></span>
          </div>
          <div class="flex items-center justify-between text-sm">
      ...
      <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-5">
        <h3 class="text-sm font-semibold text-gray-500 uppercase tracking-wider mb-3">Environment</h3>
        <div class="space-y-2">
          <button @click="$router.push(`/sites/${site.id}/settings`)" class="w-full text-left text-sm text-blue-600 hover:text-blue-800 font-medium">Edit .env file</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showAddDaemon" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
          <h3 class="text-lg font-bold text-gray-900">Add Background Process</h3>
          <button @click="showAddDaemon = false" class="text-gray-400 hover:text-gray-600">
            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>
        <div class="p-6 space-y-4">
          <div>
            <label class="block text-gray-700 text-sm font-bold mb-1">Command</label>
            <input v-model="newDaemon.command" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow text-sm" placeholder="php artisan queue:work">
          </div>
          <div>
            <label class="block text-gray-700 text-sm font-bold mb-1">Directory</label>
            <input v-model="newDaemon.directory" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow text-sm" :placeholder="site.path">
          </div>
          <div class="flex justify-end gap-3 pt-2">
            <button @click="showAddDaemon = false" class="px-4 py-2 text-gray-700 bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-50 font-semibold text-sm transition-colors">Cancel</button>
    ...
    <div v-if="showAddCron" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
          ...
          <button @click="showAddCron = false" class="text-gray-400 hover:text-gray-600">
            <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </div>
        <div class="p-6 space-y-4">
          <div>
            <label class="block text-gray-700 text-sm font-bold mb-1">Schedule</label>
            <input v-model="newCron.expression" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow text-sm font-mono" placeholder="* * * * *">
          </div>
          <div>
            <label class="block text-gray-700 text-sm font-bold mb-1">Command</label>
            <input v-model="newCron.command" class="w-full border border-gray-200 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow text-sm" placeholder="php artisan schedule:run">
          </div>
          <div class="flex justify-end gap-3 pt-2">
            <button @click="showAddCron = false" class="px-4 py-2 text-gray-700 bg-white border border-gray-200 rounded-lg shadow-sm hover:bg-gray-50 font-semibold text-sm transition-colors">Cancel</button>
...
const statusBadge = (status: string) => {
  ...
  if (status === 'success') return `${base} bg-green-50 text-green-700 border border-green-200`;
  if (status === 'failed') return `${base} bg-red-50 text-red-700 border border-red-200`;
  if (status === 'running') return `${base} bg-yellow-50 text-yellow-700 border border-yellow-200`;
