  ...
  <div class="space-y-6">
    <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6 dark:bg-gray-900 dark:border-gray-800">
      ...
      <div class="grid grid-cols-1 gap-4">
        <div v-for="d in daemons" :key="d.id" class="border border-gray-200 rounded-lg p-5 hover:border-gray-300 transition-colors dark:border-gray-700 dark:hover:border-gray-600">
          <div class="flex justify-between items-start">
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-3 mb-2">
                <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">{{ d.name || d.command.split(' ').slice(0, 2).join(' ') }}</h3>
                <span class="px-2 py-0.5 text-xs font-semibold rounded-full"
                  :class="d.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'">
                  {{ d.status === 'active' ? 'Running' : d.status }}
                </span>
              </div>
              <p class="text-sm font-mono text-gray-600 truncate dark:text-gray-400">{{ d.command }}</p>
              <p class="text-xs text-gray-400 mt-1 dark:text-gray-500">{{ d.directory }}</p>
              <div class="flex items-center gap-3 mt-1">
                <span class="text-xs text-gray-500 dark:text-gray-400">{{ d.user || 'fluxo' }}</span>
              </div>
            </div>
            <div class="flex items-center gap-2 ml-4 shrink-0">
              <span class="text-xs text-gray-500 dark:text-gray-400">{{ d.instances || 1 }} Process{{ d.instances !== 1 ? 'es' : '' }}</span>
              <div class="relative">
                <button @click="toggleMenu(d.id)" class="px-3 py-1.5 text-xs text-gray-600 bg-gray-50 border border-gray-200 rounded-lg hover:bg-gray-100 font-medium transition-colors dark:text-gray-400 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                  Open menu
                </button>
                <div v-if="openMenu === d.id" class="absolute right-0 mt-1 w-44 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10 dark:bg-gray-900 dark:border-gray-700">
                  <button @click="restartDaemon(d.id); openMenu = null" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 text-left dark:text-gray-300 dark:hover:bg-gray-800">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                    Restart
                  </button>
                  <button @click="deleteDaemon(d.id); openMenu = null" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 text-left dark:text-red-400 dark:hover:bg-red-900/30">
...
const fetchDaemons = async (silent = false) => {
  try {
    const res = await fetch(`/api/v1/sites/${siteId}/daemons`, {
...
const restartDaemon = async (id: number) => {
  try {
    const res = await fetch(`/api/v1/sites/${siteId}/daemons/${id}/restart`, {
      method: 'POST',
      headers: { 'Authorization': `Bearer ${token()}` }
    });
    if (!res.ok) throw new Error(await res.text());
...
const deleteDaemon = async (id: number) => {
  ...
  if (!confirmed) return;
  try {
    await fetch(`/api/v1/sites/${siteId}/daemons/${id}`, {
      method: 'DELETE',
      headers: { 'Authorization': `Bearer ${token()}` }
    });
