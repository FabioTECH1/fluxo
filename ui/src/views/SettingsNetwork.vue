  ...
  <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
    <div class="flex justify-between items-center mb-4">
      <div>
        ...
        <p class="text-sm text-gray-600">Manage open ports and allowed IP addresses on your server.</p>
      </div>
          ...
          <tr v-for="rule in rules" :key="rule.id" class="hover:bg-gray-50 transition-colors">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm">
              <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full"
                    :class="rule.type === 'deny'
                      ? 'bg-red-100 text-red-800'
                      : 'bg-green-100 text-green-800'">
                {{ rule.type === 'deny' ? 'Deny' : 'Allow' }}
              </span>
            </td>
          ...
          <div class="mb-5">
            ...
            <div class="flex gap-4">
              <label class="flex items-center gap-2 cursor-pointer">
                <input type="radio" v-model="newRule.type" value="allow" class="text-blue-600 focus:ring-blue-500">
                <span class="text-sm font-medium text-gray-700">Allow</span>
                <span class="text-xs text-gray-500">Allow requests to this port.</span>
              </label>
              <label class="flex items-center gap-2 cursor-pointer">
                <input type="radio" v-model="newRule.type" value="deny" class="text-red-600 focus:ring-red-500">
                <span class="text-sm font-medium text-gray-700">Deny</span>
                <span class="text-xs text-gray-500">Deny requests to this port.</span>
              </label>
            </div>
          </div>
          <div class="mb-5">
