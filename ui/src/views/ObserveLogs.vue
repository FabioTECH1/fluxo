<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-4">
      ...
      <div class="flex flex-wrap gap-3 items-center">
        ...
        <div class="relative">
          ...
          <div v-if="showActions" class="absolute right-0 mt-1 w-48 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-10">
            <button @click="downloadLog; showActions = false" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 text-left">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
              Download
            </button>
            <button @click="clearLog; showActions = false" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 text-left">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div v-if="error" class="mb-4 text-red-700 dark:text-red-300 bg-red-50 dark:bg-red-900/30 border border-red-200 p-3 rounded-lg text-sm">
      {{ error }}
    </div>

    <div class="bg-gray-900 rounded-lg font-mono text-xs text-green-400 h-[600px] overflow-y-auto">
      <div v-if="logLines.length === 0 && !error" class="text-gray-500 dark:text-gray-400 italic p-4">No log entries found.</div>
      <div v-for="(line, idx) in logLines" :key="idx" class="flex leading-relaxed hover:bg-gray-800">
        <span class="text-gray-600 dark:text-gray-500 select-none px-3 py-0.5 text-right w-12 shrink-0 border-r border-gray-700">{{ idx + 1 }}</span>
