  ...
  <div class="bg-white rounded-lg shadow-sm border border-gray-100 p-6">
    <div class="flex justify-between items-center mb-4">
      <div>
        ...
        <p class="text-sm text-gray-600 mt-1">View recent log entries from the server.</p>
      </div>
      <div class="flex gap-3 items-center">
        <select v-model="selectedLog" @change="() => fetchLogs()" class="border border-gray-200 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-shadow">
        ...
        <div class="relative">
          <button @click="showActions = !showActions" class="px-3 py-2 text-gray-600 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 font-medium transition-colors text-sm flex items-center gap-1">
            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2zm0 7a1 1 0 110-2 1 1 0 010 2z" />
            </svg>
          </button>
          <div v-if="showActions" class="absolute right-0 mt-1 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-10">
            <button @click="downloadLog; showActions = false" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 text-left">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
              </svg>
              Download
            </button>
            <button @click="clearLog; showActions = false" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-red-600 hover:bg-red-50 text-left">
              <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    <div class="bg-gray-900 rounded-lg font-mono text-xs text-green-400 h-[600px] overflow-y-auto">
      <div v-if="logLines.length === 0 && !error" class="text-gray-500 italic p-4">No log entries found.</div>
    ...
    <div class="flex justify-between items-center mt-3">
      <span class="text-xs text-gray-500">{{ logLines.length }} lines from {{ currentLabel }}</span>
      <button @click="logLines = []" class="text-xs text-red-600 hover:text-red-900 font-semibold">Clear View</button>
    </div>
  </div>
</template>
