  ...
  <div class="space-y-6">
    <div class="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-100 dark:border-gray-800 p-6">
      <div class="flex justify-between items-center mb-4">
        <div>
          ...
          <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">
            ...
            <a href="https://supervisord.org" class="text-blue-600 dark:text-blue-400 hover:underline">Learn more</a>
          </p>
        </div>
      ...
      <div class="grid grid-cols-1 gap-4">
        <div v-for="d in daemons" :key="d.id" class="border border-gray-200 dark:border-gray-700 rounded-lg p-5 hover:border-gray-300 dark:hover:border-gray-600 transition-colors">
          <div class="flex justify-between items-start">
            <div class="flex-1 min-w-0">
              ...
              <p class="text-sm font-mono text-gray-600 dark:text-gray-400 truncate">{{ d.command }}</p>
              <p class="text-xs text-gray-400 dark:text-gray-500 mt-1">{{ d.directory }}</p>
            ...
            <div class="flex items-center gap-2 ml-4 shrink-0">
              ...
              <div class="relative">
                <button @click="toggleMenu(d.id)" class="px-3 py-1.5 text-xs text-gray-600 dark:text-gray-400 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 font-medium transition-colors">
                  Open menu
                </button>
                <div v-if="openMenu === d.id" class="absolute right-0 mt-1 w-44 bg-white dark:bg-gray-900 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700 py-1 z-10">
                  <button @click="restartDaemon(d.id); openMenu = null" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800 text-left">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>
                    Restart
                  </button>
                  <button @click="deleteDaemon(d.id); openMenu = null" class="flex items-center gap-2 w-full px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 text-left">
