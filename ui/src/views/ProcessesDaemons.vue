<template>
  <div class="space-y-6">
    <div class="bg-white dark:bg-gray-900 rounded-lg shadow-sm border border-gray-100 dark:border-gray-800 p-6">
      ...
      <div class="space-y-2">
        <div v-for="d in daemons" :key="d.id" class="flex items-center gap-3 px-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-lg hover:border-gray-300 dark:hover:border-gray-600 transition-colors">
          <span class="px-2 py-0.5 text-xs font-semibold rounded-full shrink-0" :class="d.status === 'active' ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-600'">{{ d.status === 'active' ? 'RUN' : 'STOP' }}</span>
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              <span class="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{{ d.name || d.command.split(' ').slice(0, 2).join(' ') }}</span>
              <span class="text-xs font-mono text-gray-500 dark:text-gray-400 truncate">{{ d.command }}</span>
            </div>
            <div class="flex items-center gap-3 text-xs text-gray-400 dark:text-gray-500">
              <span>{{ d.directory }}</span>
              <span>{{ d.user || 'fluxo' }}</span>
              <span>{{ d.instances || 1 }} proc</span>
              <span v-if="d.site_domain" class="text-blue-500">{{ d.site_domain }}</span>
            </div>
          </div>
...
const fetchDaemons = async (silent = false) => {
  try {
    const res = await fetch('/api/v1/daemons', { headers: { 'Authorization': `Bearer ${token()}` } });
...
const startDaemon = async (id: number) => {
  try {
    const res = await fetch(`/api/v1/daemons/${id}/start`, { method: 'POST', headers: { 'Authorization': `Bearer ${token()}` } });
    if (!res.ok) throw new Error(await res.text());
...
const stopDaemon = async (id: number) => {
  try {
    const res = await fetch(`/api/v1/daemons/${id}/stop`, { method: 'POST', headers: { 'Authorization': `Bearer ${token()}` } });
    if (!res.ok) throw new Error(await res.text());
...
const restartDaemon = async (id: number) => {
  try {
    const res = await fetch(`/api/v1/daemons/${id}/restart`, { method: 'POST', headers: { 'Authorization': `Bearer ${token()}` } });
    if (!res.ok) throw new Error(await res.text());
...
const viewLogs = async (d: any) => {
  ...
  showLogs.value = true;
  try {
    const res = await fetch(`/api/v1/daemons/${d.id}/logs`, { headers: { 'Authorization': `Bearer ${token()}` } });
...
const deleteDaemon = async (id: number) => {
  ...
  if (!confirmed) return;
  try {
    const res = await fetch(`/api/v1/daemons/${id}`, { method: 'DELETE', headers: { 'Authorization': `Bearer ${token()}` } });
    if (!res.ok) throw new Error(await res.text());
