<template>
  <BaseModal v-model="visible" :title="modalTitle" :loading="!creating && loading" :confirm-text="creating ? '' : 'Create Site'" @submit="formRef?.requestSubmit()">
    <div v-if="!creating">
    ...
    <div v-else class="space-y-1 py-2">
      ...
      <div v-for="(step, i) in progressSteps" :key="i" class="flex items-center gap-3 py-1.5">
...
const modalTitle = computed(() => creating.value ? 'Creating Site...' : 'Create New Site');
...
const startProgress = () => {
  ...
  const advance = () => {
    ...
    if (idx > 0) {
      steps[idx - 1].active = false;
      steps[idx - 1].complete = true;
    }
